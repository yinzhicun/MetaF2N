...
class FaceGAN(object):
    ...
    def process(self, img, flip=False):
        #img = cv2.resize(img, (self.in_resolution, self.in_resolution))
        img_t = self.img2tensor(img, flip)
        with torch.no_grad():
            (out, latent), noise = self.model(img_t, return_latents = True)
        out = self.tensor2img(out, flip=flip)
      
        return out

    def img2tensor(self, img, flip=False):
        img_t = torch.from_numpy(img).type(torch.FloatTensor).to(self.device)/255.
        if self.is_norm:
            img_t = (img_t - 0.5) / 0.5
        if flip:
    ...
    def tensor2img(self, img_t, pmax=255.0, imtype=np.uint8, flip=False):
        if self.is_norm:
            img_t = img_t * 0.5 + 0.5
        if flip:
            img_t = img_t.squeeze(0).permute(1, 2, 0).flip(2) # RGB->BGR
        else:
            img_t = img_t.squeeze(0).permute(1, 2, 0)
